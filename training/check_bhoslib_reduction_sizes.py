...
GRAPH_DIR = PROJECT_ROOT / "training" / "graphs" / "bhoslib"
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            if parts[0] == "p":
                ...
                G.add_nodes_from(range(1, declared_n + 1))
...
def main():
    graph_files = sorted(GRAPH_DIR.glob("*.clq"))

    if not graph_files:
        raise RuntimeError(
            f"No .clq files found in {GRAPH_DIR}"
        )

    print("=" * 120)
    print("BHOSLIB COMPLEMENT -> REDUMIS CORE SIZE CHECK")
    print("=" * 120)
    ...
    for graph_file in graph_files:
        try:
            clique_graph = load_dimacs_clique(
                graph_file
            )
