...
4. Deep instrumentation: first-incumbent depth/time, certainty evolution
    ...
    and deferred-uncertainty persistence tracking.
...
class DepthStats:
    ...
    residual_density_sum: float = 0.0
    residual_density_count: int = 0
    ...
    def record_persistence(self, persistence_score: float) -> None:
        self.persistence_sum += float(persistence_score)
        self.persistence_count += 1

    def record_residual_density(self, residual_density: float) -> None:
        self.residual_density_sum += float(residual_density)
        self.residual_density_count += 1

    @property
    def avg_persistence(self) -> float:
        return self.persistence_sum / self.persistence_count if self.persistence_count > 0 else 0.0

    @property
    def avg_residual_density(self) -> float:
        return self.residual_density_sum / self.residual_density_count if self.residual_density_count > 0 else 0.0
...
class SolverTrace:
    ...
    # Deferred-uncertainty signals tracked across DFS
    persistence_evolution: List[Tuple[int, float, float]] = field(default_factory=list)
    residual_graph_evolution: List[Tuple[int, float, float, float]] = field(default_factory=list)
    local_gain_evolution: List[Tuple[int, float]] = field(default_factory=list)
...
class BranchAndBoundSolver:
    ...
    def __init__(
        ...
        diversify_incumbents: bool = True,
        shallow_dive_limit: int = 3,
        shallow_dive_nodes: int = 2_000,
    ) -> None:
        ...
        self.diversify_incumbents = diversify_incumbents
        self.shallow_dive_limit = shallow_dive_limit
        self.shallow_dive_nodes = shallow_dive_nodes
    ...
    def solve(self, problem: MILPProblem) -> BBSolution:
        ...
        # ---- Persistent certainty / backbone-style statistics ----
        persistence_zero = np.zeros(n, dtype=float)
        persistence_one = np.zeros(n, dtype=float)
        persistence_visits = np.zeros(n, dtype=float)

        # ---- Diversified shallow incumbents ----
        best_obj = np.inf
        best_x: Optional[np.ndarray] = None
        if self.diversify_incumbents:
            dive_obj, dive_x = self._seed_diverse_incumbent(problem, features)
            if dive_x is not None and dive_obj < best_obj:
                best_obj = dive_obj
                best_x = dive_x
        ...
        while stack:
            ...
            search_progress = node.depth / max(1, n - 1)
            node_persistence = self._update_persistence(
                x,
                persistence_zero,
                persistence_one,
                persistence_visits,
            )
            ds.record_persistence(node_persistence)
            ds.record_residual_density(graph_state.residual_edge_density)
            trace.persistence_evolution.append((node.depth, float(node_persistence), float(search_progress)))
            trace.residual_graph_evolution.append(
                (
                    node.depth,
                    float(graph_state.active_vertex_count),
                    float(graph_state.residual_edge_density),
                    float(graph_state.residual_degree.mean() if graph_state.active_vertex_count > 0 else 0.0),
                )
            )

            local_gain = np.array([graph_state.propagation_gain(i) for i in range(n)], dtype=float)
            trace.local_gain_evolution.append((node.depth, float(local_gain[graph_state.active].mean() if graph_state.active_vertex_count > 0 else 0.0)))
            ...
            branch_var = self.branching.select(
                ...
                certainty_persistence=self._current_persistence_score(
                    persistence_zero,
                    persistence_one,
                    persistence_visits,
                ),
    ...
    def _current_persistence_score(
        self,
        persistence_zero: np.ndarray,
        persistence_one: np.ndarray,
        persistence_visits: np.ndarray,
    ) -> np.ndarray:
        denom = np.maximum(persistence_visits, 1.0)
        return np.maximum(persistence_zero, persistence_one) / denom
    ...
    def _update_persistence(
        ...
        near_zero = x <= near_tol
        near_one = x >= 1.0 - near_tol
        persistence_visits += 1.0
        persistence_zero[near_zero] += 1.0
        persistence_one[near_one] += 1.0
        score = np.maximum(persistence_zero, persistence_one) / np.maximum(persistence_visits, 1.0)
        return float(score.mean())

    def _seed_diverse_incumbent(self, problem: MILPProblem, features: VertexFeatures | None) -> tuple[float, Optional[np.ndarray]]:
        """Run a few shallow dives with perturbed weights to find early incumbents."""
        if problem.n_vars <= 0:
            return np.inf, None

        base = CertaintyFirstConfig()
        perturbations = [
            base,
            CertaintyFirstConfig(alpha=0.38, beta=0.24, gamma=0.14, delta=0.10, persistence=0.08, local_gain=0.06, depth_decay=0.45, fallback_mix=0.20),
            CertaintyFirstConfig(alpha=0.28, beta=0.20, gamma=0.22, delta=0.10, persistence=0.12, local_gain=0.10, depth_decay=0.65, fallback_mix=0.25),
        ]

        shallow_nodes = min(self.shallow_dive_nodes, max(250, self.shallow_dive_nodes))
        best_obj = np.inf
        best_x: Optional[np.ndarray] = None

        for cfg in perturbations[: self.shallow_dive_limit]:
            helper = BranchAndBoundSolver(
                branching=CertaintyFirstBranching(config=cfg),
                tolerance=self.tol,
                max_nodes=shallow_nodes,
                use_reductions=self.use_reductions,
                certainty_lambda=self.certainty_lambda,
                feature_engine=self.feat_engine,
                track_backbone=False,
                pseudo_cost_min_obs=self.pseudo_cost_min_obs,
                diversify_incumbents=False,
                shallow_dive_limit=0,
                shallow_dive_nodes=0,
            )
            sol = helper.solve(problem)
            if sol.x is not None and sol.objective_value < best_obj:
    ...
    def _apply_reductions(
        ...
        # 4. Certainty-guided exact reductions: use the current LP solution
        # to trigger exact simplifications, but never force on certainty alone.
        if not _add(Reductions.degree_one_lp_agree(lp_x, graph_state, graph_state._adj)):
            return []
