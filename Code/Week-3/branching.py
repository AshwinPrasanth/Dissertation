...
The main branching score is intentionally small and hypothesis-driven:
    ...
    score(v) = alpha * LP_certainty(v)
             ...
             + delta * pseudoCost(v)
             + eta   * certaintyPersistence(v)
             + zeta  * localReductionGain(v)
...
class CertaintyFirstConfig:
    ...
    alpha: float = 0.32       # LP certainty  |x_v - 0.5|
    beta: float = 0.22        # MWUA certainty
    gamma: float = 0.16       # residual degree / local density
    delta: float = 0.10       # pseudo-cost
    persistence: float = 0.12 # certainty persistence across DFS
    local_gain: float = 0.08  # local reduction / propagation gain
    depth_decay: float = 0.55 # how fast certainty weights decay with depth
    fallback_mix: float = 0.20 # blend toward simpler structure as depth grows
...
class CertaintyFirstBranching(BranchingStrategy):
    ...
    def select(
        ...
        tol: float,
        depth: int,
        search_progress: float | None = None,
        certainty_persistence: np.ndarray | None = None,
        ...
        progress = float(np.clip(search_progress if search_progress is not None else depth / max(len(lp_x), 1), 0.0, 1.0))
        certainty_scale = max(0.25, 1.0 - self.cfg.depth_decay * progress)
        fallback_scale = self.cfg.fallback_mix * progress

        # LP certainty
        lp_cert = np.abs(lp_x - 0.5)
        scores += self.cfg.alpha * certainty_scale * (lp_cert / 0.5)   # normalise to [0, 1]

        # MWUA certainty
        if features is not None:
            scores += self.cfg.beta * certainty_scale * features.mwua_certainty

        # Residual degree ratio
        if graph_state is not None:
            rd_ratio = graph_state.residual_degree_ratio
            for i in frac:
                if graph_state.active[i]:
                    local_signal = 0.45 * rd_ratio[i] + 0.30 * graph_state.local_density(i) + 0.25 * graph_state.propagation_gain(i)
                    scores[i] += self.cfg.gamma * local_signal
        ...
        # Certainty persistence across the DFS.
        if certainty_persistence is not None:
            for i in frac:
                scores[i] += self.cfg.persistence * certainty_scale * float(certainty_persistence[i])

        # Local reduction gain: favour nodes likely to create exact simplifications.
        if local_gain is not None:
            for i in frac:
                scores[i] += self.cfg.local_gain * float(local_gain[i])

        # As the search progresses, blend a small amount of simpler structure.
        if graph_state is not None and fallback_scale > 0.0:
            fallback = np.array([graph_state.active_neighbor_ratio(i) + graph_state.local_density(i) for i in range(len(lp_x))], dtype=float)
            scores += fallback_scale * fallback
...
class DegreeBranching(BranchingStrategy):
    ...
    def select(
        ...
        if graph_state is not None:
            ...
            local_score = rd.copy()
            for i in frac:
                local_score[i] += 0.25 * graph_state.propagation_gain(i)
            return int(frac[np.argmax(local_score[frac])])
        return int(frac[0])
