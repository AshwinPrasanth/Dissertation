...
7. run_deferred_uncertainty_analysis()
    Compare certainty persistence, residual graph evolution, and progress-aware
    collapse dynamics across certainty-first and simpler baselines.
...
def _mean_from_pairs(pairs: List[Tuple], index: int) -> float:
    if not pairs:
        return 0.0
    return float(np.mean([float(row[index]) for row in pairs]))
...
def run_strategy_comparison(
        ...
        for trial in range(n_trials):
            ...
            for strat in strategies:
                ...
                rows.append({
                    ...
                    "mean_persistence": f"{_mean_from_pairs(tr.persistence_evolution, 1):.4f}",
                    "mean_residual_density": f"{_mean_from_pairs(tr.residual_graph_evolution, 2):.4f}",
                    "mean_active_vertices": f"{_mean_from_pairs(tr.residual_graph_evolution, 1):.2f}",
                    "mean_local_gain": f"{_mean_from_pairs(tr.local_gain_evolution, 1):.4f}",
                })
    ...
    _write_csv(out_dir / "strategy_comparison.csv",
               ["n","trial","strategy","status","obj","explored","pruned",
                ...
                "reduction_fixes","lp_time_frac","mean_persistence",
                "mean_residual_density","mean_active_vertices","mean_local_gain"],
               rows)
...
# ---------------------------------------------------------------------------
# 3b. Deferred-uncertainty analysis
# ---------------------------------------------------------------------------

def run_deferred_uncertainty_analysis(
    out_dir: Path,
    n: int = 35,
    p: float = 0.4,
    seed: int = 7,
    ...
    """Analyse certainty persistence, residual graph collapse, and progress.

    This is the most direct Week 2 diagnostic for the deferred-uncertainty
    hypothesis: do variables become easier to decide as we go deeper, and do
    the residual graph statistics collapse in step with certainty?
    """
    _, problem = _build_gnp(n, p, seed)
    strategies_to_compare = ["certainty_first", "most_fractional", "mwua_only"]
    rows: List[dict] = []

    fig, axes = plt.subplots(2, 2, figsize=(14, 9))
    fig.suptitle(f"Deferred-Uncertainty Dynamics (n={n}, p={p})", fontsize=14)
    ...
    for strat in strategies_to_compare:
        ...
        def _bin_mean(pairs: List[Tuple[int, float]], depth_index: int = 0, value_index: int = 1) -> Dict[int, List[float]]:
            from collections import defaultdict
            bucket: Dict[int, List[float]] = defaultdict(list)
            for row in pairs:
                bucket[int(row[depth_index])].append(float(row[value_index]))
            return bucket

        certainty_by_depth = _bin_mean(tr.certainty_evolution, 0, 1)
        persistence_by_depth = _bin_mean(tr.persistence_evolution, 0, 1)
        progress_by_depth = _bin_mean(tr.persistence_evolution, 0, 2)
        residual_by_depth = _bin_mean(tr.residual_graph_evolution, 0, 2)
        active_by_depth = _bin_mean(tr.residual_graph_evolution, 0, 1)
        local_gain_by_depth = _bin_mean(tr.local_gain_evolution, 0, 1)

        for depth in sorted(set(certainty_by_depth) | set(persistence_by_depth) | set(residual_by_depth)):
            rows.append({
                "strategy": strat,
                "depth": depth,
                "mean_lp_certainty": np.mean(certainty_by_depth.get(depth, [0.0])),
                "mean_persistence": np.mean(persistence_by_depth.get(depth, [0.0])),
                "mean_progress": np.mean(progress_by_depth.get(depth, [0.0])),
                "mean_residual_density": np.mean(residual_by_depth.get(depth, [0.0])),
                "mean_active_vertices": np.mean(active_by_depth.get(depth, [0.0])),
                "mean_local_gain": np.mean(local_gain_by_depth.get(depth, [0.0])),
            })

        depths = sorted(certainty_by_depth.keys())
        if depths:
            axes[0, 0].plot(depths, [np.mean(certainty_by_depth[d]) for d in depths], marker=MARKERS[strat], color=PALETTE[strat], label=strat.replace("_", " "), linewidth=1.8)
            axes[0, 1].plot(depths, [np.mean(persistence_by_depth.get(d, [0.0])) for d in depths], marker=MARKERS[strat], color=PALETTE[strat], label=strat.replace("_", " "), linewidth=1.8)
            axes[1, 0].plot(depths, [np.mean(residual_by_depth.get(d, [0.0])) for d in depths], marker=MARKERS[strat], color=PALETTE[strat], label=strat.replace("_", " "), linewidth=1.8)
            axes[1, 1].plot(depths, [np.mean(active_by_depth.get(d, [0.0])) for d in depths], marker=MARKERS[strat], color=PALETTE[strat], label=strat.replace("_", " "), linewidth=1.8)

    axes[0, 0].set_title("LP Certainty vs Depth")
    axes[0, 1].set_title("Certainty Persistence vs Depth")
    axes[1, 0].set_title("Residual Edge Density vs Depth")
    axes[1, 1].set_title("Active Vertices vs Depth")
    ...
    axes[0, 0].set_ylabel("Mean LP Certainty")
    axes[0, 1].set_ylabel("Mean Persistence")
    axes[1, 0].set_ylabel("Residual Edge Density")
    axes[1, 1].set_ylabel("Active Vertices")

    fig.tight_layout()
    fig.savefig(out_dir / "deferred_uncertainty_analysis.png", dpi=220, bbox_inches="tight")
    plt.close(fig)

    _write_csv(
        out_dir / "deferred_uncertainty_analysis.csv",
        [
            "strategy",
            "depth",
            "mean_lp_certainty",
    ...
    print("[deferred_uncertainty_analysis] saved")
...
def run_all(out_dir: Optional[Path] = None, quick: bool = False) -> None:
    ...
    if quick:
        ...
        run_deferred_uncertainty_analysis(out_dir, n=20, max_nodes=30_000)
        ...
        run_deferred_uncertainty_analysis(out_dir)
