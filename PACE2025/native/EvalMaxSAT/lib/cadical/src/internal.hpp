...
struct Internal {
  ...
  unsigned int original_max_var = 0;
  std::vector<double> vertex_features;
  std::vector<unsigned int> vertex_mapping;
  std::vector<std::vector<unsigned int>> vertex_edges;
  ...
  void collect_branch_sample (int decision_variable);
  int64_t get_decisions () const { return stats.decisions; }
  int64_t get_conflicts () const { return stats.conflicts; }
  int64_t get_propagations () const { return stats.propagations.search; }
  ...
  void set_original_max_var (unsigned int n) {
  original_max_var = n;
  }

  void setVertexFeatures(const std::vector<double>& features) {
  vertex_features = features;
}

  void set_vertex_mapping (
    const std::vector<unsigned int>& mapping,
    const std::vector<std::vector<unsigned int>>& edges) {
...
inline bool Internal::search_limits_hit ()
{
  ...
  if(lim.time_point_timeout < std::chrono::steady_clock::now()) {
      LOG ("timeout limit %" PRId64 " reached", lim.timeout);
      return true;
  }
