...
public:
    ...
    solver->internal->setVertexFeatures(features);
}

    void setMLBranch(bool enabled) {
    solver->internal->opts.mlbranch = enabled ? 1 : 0;
}

    void printStats() {
    std::cerr << "[STATS] decisions=" << solver->internal->get_decisions()
              << " conflicts=" << solver->internal->get_conflicts()
              << " propagations=" << solver->internal->get_propagations()
              << std::endl;
}

    void setOriginalMaxVar(unsigned int n) {
    original_max_var = n;
    solver->internal->set_original_max_var(n);
}

    void setVertexMapping(
    const std::vector<unsigned int>& mapping,
    const std::vector<std::vector<unsigned int>>& edges) {
    solver->internal->set_vertex_mapping(mapping, edges);
}
