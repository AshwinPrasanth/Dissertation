...
public:
  ...
  bool solve() {
    ...
    std::cerr << "[BOUNDARY DEBUG] solver nVars = "
          << solver->nVars() << std::endl;
    std::cerr << "[BOUNDARY DEBUG] _poids size = "
          << _poids.size() << std::endl;

    const unsigned int original_max_var = solver->nVars();
    solver->setOriginalMaxVar(original_max_var);
